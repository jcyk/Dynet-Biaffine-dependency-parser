...
class NotagParser_auxemb(object):
	def __init__(self, vocab,
		...
		self.aux_pret_word_embs = pc.lookup_parameters_from_numpy(vocab.get_aux_pret_embs())
		
		self.LSTM_builders = []
		f = orthonormal_VanillaLSTMBuilder(1, word_dims + aux_word_dims, lstm_hiddens, pc, randn_init)
		b = orthonormal_VanillaLSTMBuilder(1, word_dims + aux_word_dims, lstm_hiddens, pc, randn_init)
	...
	def run(self, word_inputs, arc_targets = None, rel_targets = None, isTrain = True):
		...
		emb_inputs = [ dy.concatenate([w, aw]) for w, aw in zip(word_embs,aux_word_embs)]
