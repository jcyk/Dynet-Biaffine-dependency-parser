...
if __name__ == "__main__":
	...
	argparser.add_argument('--model', default='NotagParser_auxemb')
	args, extra_args = argparser.parse_known_args()
	config = Configurable(args.config_file, extra_args)
	Parser = getattr(models, args.model)

	vocab = Vocab(config.train_file, config.pretrained_embeddings_file, config.min_occur_count, config.aux_pretrained_embeddings_file)
	...
	parser = Parser(vocab, config.word_dims, config.aux_word_dims, config.dropout_emb, config.lstm_layers, config.lstm_hiddens, config.dropout_lstm_input, config.dropout_lstm_hidden, config.mlp_arc_size, config.mlp_rel_size, config.dropout_mlp)
	...
	while global_step < config.train_iters:
		...
		epoch += 1
		for words, _, arcs, rels in data_loader.get_batches(batch_size = config.train_batch_size, shuffle = True):
			dy.renew_cg()
			arc_accuracy, rel_accuracy, overall_accuracy, loss = parser.run(words, arcs, rels)
