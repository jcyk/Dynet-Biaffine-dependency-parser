...
if __name__ == "__main__":
	...
	argparser.add_argument('--critic_scale', type=float, default = 0.005)
	...
	#parser.set_trainable_flags(train_emb = False, train_lstm = True, train_critic = True, train_score = False)
	#global_step = 0
	#while global_step < 50000:
	#	for words, tags, arcs, rels in data_loader.get_batches(batch_size = config.train_batch_size):
	#		dy.renew_cg()
	...
	while global_step < config.train_iters:
		...
		for words, tags, arcs, rels in data_loader.get_batches(batch_size = config.train_batch_size):
			dy.renew_cg()
			if inner_step % (args.ncritic + 1) == 0 and global_step > 100:
